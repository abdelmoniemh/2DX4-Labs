void SysTick_Wait05ms(uint32_t delay){
uint32_t i;
for(i=0; i<delay; i++){
SysTick_Wait(60000); // wait 10ms (assumes 120 MHz clock)
}
}
...
SYSCTL_RCGCGPIO_R |= 0b00010000; // 1.
activate clock for port E
while ((SYSCTL_PRGPIO_R & 0b00010000) == 0) {} // wait for clock/port to be
ready
...
// 5) enable analog function on PE4
SYSCTL_RCGCADC_R |= 0x01;
// 6) activate ADC0
ADC0_PC_R = 0x01;
// 7) maximum speed is 125K samples/sec
ADC0_SSPRI_R = 0x0123;
...
SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R12; // activate clock for
Port N
while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R12) == 0){}; // allow time for clock to stabilize
GPIO_PORTN_DIR_R |= 0x01;
// make PN0 out (PN0 built-in LED1)
GPIO_PORTN_AFSEL_R &= ~0x01; //
disable alt funct on PN0
GPIO_PORTN_DEN_R |= 0x01; //
enable digital I/O on PN0
...
uint32_t func_debug[500];
// array used to store realtime data
int sum = 0;
int average = 0;
int main(void){
uint32_t delay = 1;
// this is a multiplier used for SysTick = half of delay
uint32_t count = 0;
// my loop control variable initialized to 0
PLL_Init();
// Default Set System Clock to
120MHz
SysTick_Init();
// Initialize SysTick configuration
ADC_Init();
// Initialize ADC as per
Vavano text
PortN_Init();
// Initialize Port N GPIO
while(count < 500){
// run program for fixed number of steps/samples
GPIO_PORTN_DATA_R ^= 0b00000001; //
toggle LED for visualization of process
SysTick_Wait05ms(delay);
// halfdelay*10ms
func_debug[count++] = ADC0_InSeq3(); // Acquire
sample
GPIO_PORTN_DATA_R ^= 0b00000001; // //
toggle LED for visualization of process
SysTick_Wait05ms(delay);
